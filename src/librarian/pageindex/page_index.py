...
def toc_detector_single_page(content, model=None):
    ...
    result = json_content.get('toc_detected')
    if result is None:
        logging.warning(f"toc_detector_single_page: missing 'toc_detected' key. Raw response: {repr(response[:300])}")
        return 'no'
    return result
...
def page_list_to_group_text(page_contents, token_lengths, max_tokens=20000, overlap_page=1):    
    ...
    _log.info(f'divide page_list to groups {len(subsets)}')
    return subsets
...
async def fix_incorrect_toc(toc_with_page_number, page_list, incorrect_results, start_index=1, model=None, logger=None):
    _log.info(f'start fix_incorrect_toc with {len(incorrect_results)} incorrect results')
    ...
    for item, result in zip(incorrect_results, results):
        if isinstance(result, LLMCallError):
            raise result
        if isinstance(result, Exception):
            _log.info(f"Processing item {item} generated an exception: {result}")
            continue
...
async def fix_incorrect_toc_with_retries(toc_with_page_number, page_list, incorrect_results, start_index=1, max_attempts=3, model=None, logger=None):
    ...
    while current_incorrect:
        _log.info(f"Fixing {len(current_incorrect)} incorrect results")
...
async def verify_toc(page_list, list_result, start_index=1, N=None, model=None):
    ...
    _log.info(f"accuracy: {accuracy*100:.2f}%")
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    _log.info(mode)
    _log.info(f'start_index: {start_index}')
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None):
    ...
    if node['end_index'] - node['start_index'] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        _log.info(f"large node: {node['title']} start_index: {node['start_index']} end_index: {node['end_index']} token_num: {token_num}")
...
def page_index_main(doc, opt=None):
    ...
    async def page_index_builder():
        ...
        if opt.if_add_node_text == 'yes':
            # Labeled variant interleaves <physical_index_N> page markers so the
            # consumer can split node text back into per-page records.
            if opt.if_add_node_text_labels == 'yes':
...
def validate_and_truncate_physical_indices(toc_with_page_number, page_list_length, start_index=1, logger=None):
    ...
    _log.info(f"Document validation: {page_list_length} pages, max allowed index: {max_allowed_page}")
    if truncated_items:
        _log.info(f"Truncated {len(truncated_items)} TOC items that exceeded document length")
